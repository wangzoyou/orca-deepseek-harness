...
要求 Node.js `22.19+` 或 `24+`、pnpm，以及 npm。
...
安装脚本会安装依赖、构建 Harness CLI，并让 TUI 指向仓库内的 Harness。然后可以运行：
...
DSH 会话存储在 `~/.dsh/sessions` 下。每条事件是一个独立 zstd frame，因此 Orca 集成解析器会连续解压所有 frame，而不是只解压第一个 frame。会话恢复命令为：

```sh
dsh cli --resume <session-id>
```

上游提交、许可证和集成约定记录在 [`BUNDLE_MANIFEST.json`](BUNDLE_MANIFEST.json)；第三方声明见 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)。
