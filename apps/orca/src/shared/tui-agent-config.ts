...
export const TUI_AGENT_CONFIG: Record<TuiAgent, TuiAgentConfig> = {
  ...
  'deepseek-harness': {
    ...
    // — an interactive REPL over the official Harness runtime; `dsh --profile tui`
    // The explicit `cli` route is required so wrapper options and positional prompts
    // are parsed by dsh-terminal-plugin instead of delegated to the official binary.
    launchCmd: 'dsh cli',
    expectedProcess: 'dsh',
